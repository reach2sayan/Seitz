...
// mesh), 3D path. Reciprocal-space rotations are the transpose of the real-space
// rotations.
...
// Irreducible mesh from a reciprocal point group (the rotation-set form).
// `rot_reciprocal` must already be the reciprocal group (time reversal, if
// wanted, baked in by the caller).
...
// requested), then reduce the mesh. Errors via get_dataset
// (e_spacegroup_search_failed / …). For the object-oriented entry point, prefer
