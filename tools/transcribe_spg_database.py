...
The output is split into two headers so the encoded-operation packing (a
compile-time-only compaction) never reaches a public header:
  - <metadata_out>: SpacegroupTypeRaw + kSpacegroupTypes (plain metadata; backs
    the constexpr catalog in the public spg_database.hpp).
  - <ops_out>: kSymmetryOperations + kSymmetryOperationIndex (the encoded ops;
    included only by src/data/spg_database.cpp, which decodes them).

Usage: transcribe_spg_database.py <spg_database.c> <metadata_out.hpp> <ops_out.hpp>
"""
import re
import sys
...
def main():
    ...
    # Layer groups: appended after the 530 space-group settings via a negative
    # hall-number convention (settings -1..-116 for the 80 layer groups). Same
    # struct layout; the operation index offsets into the SAME symmetry_operations
    # array parsed above. 117 entries each (index 0 is the zero sentinel).
    lg_body = extract_array_body(text, "layer_group_types[]")
    lg_entries = list(parse_entries(strip_comments(lg_body)))[:117]
    assert len(lg_entries) == 117, len(lg_entries)
    lg_rows = []
    for e in lg_entries:
        f = split_top_level(e)
        assert len(f) == 9, (len(f), e)
        ...
        centering = CENTERING[f[7].strip()]
        pg = int(f[8])
        lg_rows.append((number, strs, centering, pg))

    lg_idx_body = extract_array_body(text, "layer_symmetry_operation_index[][2]")
    lg_idx_entries = list(parse_entries(strip_comments(lg_idx_body)))[:117]
    assert len(lg_idx_entries) == 117, len(lg_idx_entries)
    lg_idx = []
    for e in lg_idx_entries:
        a, b = [int(x) for x in e.split(",")]
        lg_idx.append((a, b))
    ...
    # --- metadata header (public): plain space-group metadata, no compaction ---
    with open(meta_out, "w") as fh:
        w = fh.write
        w("#pragma once\n\n")
        w(gen_banner)
        w("//\n")
        w("// Space-group metadata only (no encoded operations) so it can back the\n")
        ...
        w("// Layer-group metadata (80 layer groups, 117 settings incl. the zero\n")
        w("// sentinel). Indexed by the negation of the (negative) layer hall number.\n")
        w("inline constexpr std::array<SpacegroupTypeRaw, %d> kLayerGroupTypes = {{\n" % len(lg_rows))
        for (number, strs, centering, pg) in lg_rows:
            lits = ", ".join(cpp_string(s) for s in strs)
            w("    {%d, %s, %d, %d},\n" % (number, lits, centering, pg))
        w("}};\n\n")
        w("} // namespace cppcrystal::data\n")
    ...
    with open(ops_out, "w") as fh:
        ...
        w(gen_banner)
        w("//\n")
        w("// Encoded symmetry operations (the compile-time-only compaction):\n")
        ...
        w("// Layer-group operation index ({count, offset} into kSymmetryOperations,\n")
        w("// the same shared array). Indexed by -(layer hall number); 117 entries.\n")
        w("inline constexpr std::array<std::array<int, 2>, %d> kLayerSymmetryOperationIndex = {{\n" % len(lg_idx))
        for (a, b) in lg_idx:
            w("    {{%d, %d}},\n" % (a, b))
        w("}};\n\n")
        w("} // namespace cppcrystal::data\n")
    ...
    print("spacegroup_types: %d, symmetry_operations: %d, index: %d, "
          ...
          (len(sg_rows), len(ops), len(idx), len(lg_rows), len(lg_idx)))
