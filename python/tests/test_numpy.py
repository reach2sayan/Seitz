...
def test_positions_are_c_contiguous_float64(bcc_fe: cc.Cell) -> None:
...
def test_types_are_an_int32_array(bcc_fe: cc.Cell) -> None:
    types = bcc_fe.types
...
def test_the_matrix_property_hands_back_a_copy() -> None:
    ...
    lattice = cc.Lattice(np.eye(3) * 2.0)
    first = lattice.matrix
    first[0, 0] = 99.0
    assert lattice.matrix[0, 0] == 2.0


def test_positions_accept_a_plain_nested_list(bcc_fe: cc.Cell) -> None:
    from_list = cc.Cell(cc.Lattice(np.eye(3)), [[0.0, 0.0, 0.0]], [1])
