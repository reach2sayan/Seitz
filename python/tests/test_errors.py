...
do today -- ``libcppcrystal.so`` exports them ``STB_GNU_UNIQUE``, one instance
...
the library (``src/core/validation.hpp``, compiled into ``libcppcrystal.so``) and
...
def test_from_basis_answers_none_where_the_constructor_raises() -> None:
    ...
    assert cc.Lattice.from_basis(singular) is None
...
def test_an_empty_cell_raises_the_specific_class() -> None:
    ...
    ``e_empty_cell`` is created inside libcppcrystal.so and matched by a handler
    ...
    as a bare CppCrystalError and only a specific assertion would notice.
    """
    empty = cc.Cell(cc.Lattice(np.eye(3)), np.zeros((0, 3)), [])
    with pytest.raises(errors.EmptyCellError):
        cc.analyze(empty).hall
...
def test_an_unknown_wyckoff_letter_raises_with_a_message() -> None:
    ...
    with pytest.raises(errors.CppCrystalError, match="Wyckoff"):
        group.wyckoff("z")


def test_the_hierarchy_is_rooted_at_cppcrystalerror() -> None:
    for name in errors.__all__:
        cls = getattr(errors, name)
        assert issubclass(cls, errors.CppCrystalError)
    assert issubclass(errors.CppCrystalError, Exception)


def test_an_out_of_range_group_number_raises() -> None:
    with pytest.raises(errors.CppCrystalError):
        cc.SpaceGroup.from_number(cc.GroupFamily.space, 231)
