...
def test_read_cif_takes_a_path(tmp_path) -> None:
    ...
    from_path = pyseitz.read_cif(path)
    from_text = pyseitz.read_cif(NACL)
...
def test_read_cif_defaults_to_the_cif_tolerance() -> None:
    ...
    assert pyseitz.CIF_SYMPREC == pytest.approx(1e-3)
    assert pyseitz.CIF_SYMPREC > pyseitz.K_DEFAULT_SYMPREC
    # An explicit tolerance is accepted as a model or a plain dict.
    assert len(pyseitz.read_cif(NACL, {"symprec": 1e-3})) == 1
    assert len(pyseitz.read_cif(NACL, pyseitz.Tolerance(symprec=1e-3))) == 1
...
def test_a_shared_site_is_reported_not_hidden() -> None:
    ...
    (structure,) = pyseitz.read_cif(text)
...
def test_write_cif_p1_round_trips(bcc_fe) -> None:
    text = pyseitz.write_cif(bcc_fe, name="written")
    (back,) = pyseitz.read_cif(text)
...
def test_write_cif_symmetrized_names_its_group(bcc_fe) -> None:
    analyzer = pyseitz.analyze(bcc_fe)
    text = pyseitz.write_cif(analyzer, name="determined")
    assert "_space_group_symop_operation_xyz" in text
    assert "_atom_site_Wyckoff_symbol" in text
    (back,) = pyseitz.read_cif(text)
    assert back.hall is not None
    assert (
        pyseitz.spacegroup_type(back.hall).number
...
def test_write_cif_writes_a_file(bcc_fe, tmp_path) -> None:
    ...
    text = pyseitz.write_cif(bcc_fe, path=path)
    assert path.read_text() == text


def test_write_cif_rejects_anything_else() -> None:
    with pytest.raises(TypeError, match="Cell or a SymmetryAnalyzer"):
        pyseitz.write_cif("not a cell")


def test_symmetry_operation_xyz_round_trips() -> None:
    for op in pyseitz.operations_from_database(pyseitz.default_hall(pyseitz.GroupFamily.space, 227)):
        back = pyseitz.SymmetryOperation.from_xyz(op.to_xyz())
        assert pyseitz.same_operation(back, op, 1e-9)


def test_symmetry_operation_from_xyz_reads_what_files_write() -> None:
    op = pyseitz.SymmetryOperation.from_xyz("1/2+x, -y, z")
    assert np.array_equal(op.rotation, np.diag([1, -1, 1]))
    assert np.allclose(op.translation, [0.5, 0.0, 0.0])
    assert pyseitz.SymmetryOperation.from_xyz("x,y,z").to_xyz() == "x,y,z"
