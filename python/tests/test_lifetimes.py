...
-DSEITZ_ENABLE_SANITIZERS=ON at least once per change to the bindings.
"""
...
def test_a_wyckoff_outlives_the_name_that_reached_it() -> None:
    ...
    group = sz.SpaceGroup.from_number(sz.GroupFamily.space, 225)
...
def test_cell_atoms_survives_the_cell(bcc_fe: sz.Cell) -> None:
...
def test_a_dataset_outlives_its_analyzer(bcc_fe: sz.Cell) -> None:
...
def test_operations_iterate_safely(bcc_fe: sz.Cell) -> None:
    operations = sz.analyze(bcc_fe).operations
    gc.collect()
