...
turns such a result into a raised :class:`~seitz.errors.SeitzError` subclass,
...
Nothing about the raising surface changes; ``seitz.read_cif`` still raises.  The
...
Only :class:`~seitz.errors.SeitzError` becomes an ``Err``.  A ``TypeError``, an
...
def attempt(call: Callable[[], _T]) -> Result[_T, SeitzError]:
    """Run ``call``, keeping a :class:`~seitz.errors.SeitzError` as a value.
    ...
    there is no function to wrap, only an access to defer::
        ...
        results.attempt(lambda: seitz.Lattice(basis))
    """
    try:
        return Ok(call())
...
# The fallible entry points of `seitz`, pre-wrapped. These three are the whole
